...
class KickStarterService implements SingletonInterface
{
     ...
     * @var array
     */
    protected $extensionRemovals = [

        EnterpriseLevelEnumeration::BY_DEFAULT => [],
        EnterpriseLevelEnumeration::MINIMALIST => [
            'about', 'aboutmodules', 'belog', 'beuser', 'context_help', 'felogin', 'form', 'impexp',
            'info_pagetsconfig', 'info', 'reports', 'setup', 'sys_note', 'viewpage', 'wizard_crpages',
            'wizard_sortpages', 'func', 'documentation', 'lowlevel'
        ],
        EnterpriseLevelEnumeration::SMALL => [
        ...
        EnterpriseLevelEnumeration::MEDIUM => [
            'about', 'aboutmodules', 'context_help', 'impexp', 'info_pagetsconfig', 'wizard_crpages',
            'wizard_sortpages', 'func', 'documentation'
        ],
        EnterpriseLevelEnumeration::LARGE => [
     ...
     * @var array
     */
    protected $extensionAdditions = [
        EnterpriseLevelEnumeration::BY_DEFAULT => [],
        EnterpriseLevelEnumeration::MINIMALIST => [],
        EnterpriseLevelEnumeration::SMALL => [],
        EnterpriseLevelEnumeration::MEDIUM => ['scheduler', 'recycler', 'filemetadata'],
        EnterpriseLevelEnumeration::LARGE => [
            'scheduler',
            'taskcenter',
            'sys_action',
    ...
    protected function getAuthor($author)
    {
        ...
        if (true === isset($GLOBALS['BE_USER']) && true === isset($GLOBALS['BE_USER']->user['name'])) {
            return $GLOBALS['BE_USER']->user['name'] .
                isset($GLOBALS['BE_USER']->user['email']) ? '<' . $GLOBALS['BE_USER']->user['email'] . '>' : '';
        }
     ...
     * @return string|NULL
     */
    public function generateFluidPoweredSite(
        ...
        if (false === ExtensionManagementUtility::isLoaded($extensionKey)) {
            $this->kickstartProviderExtension(
                $extensionKey,
                $vendorExtensionKey,
                $author,
                $title,
        ...
        $removals = $this->extensionRemovals[EnterpriseLevelEnumeration::MINIMALIST];
        foreach (array_reverse($removals) as $installExtensionKey) {
        ...
        return 'The steps you selected in your configuration have been performed: your Provider Extension is now ' .
            'ready for use and can be accessed in the extension folder or through the file list module if you ' .
            'selected to create a file mount point.';
    }
    ...
    {
        $query = 'INSERT INTO `pages` (`pid`, `tstamp`, `crdate`, `hidden`, `title`, `doktype`, `is_siteroot`, ' .
            '`backend_layout`, `backend_layout_next_level`, `tx_fed_page_controller_action`, ' .
            '`tx_fed_page_controller_action_sub`) VALUES ' .
            '(%d, %d, %d, 1, \'%s\', %s, %s, \'fluidpages__fluidpages\', \'fluidpages__fluidpages\', %s, %s);';
        return sprintf(
            $query,
            $pid,
            time(),
    ...
    {
        $query = 'INSERT INTO `sys_template` (`pid`, `tstamp`, `crdate`, `title`, `sitetitle`, `root`, ' .
            '`include_static_file`) VALUES (%d, %d, %d, \'ROOT\', \'%s\', 1, ' .
            '\'EXT:fluidcontent_core/Configuration/TypoScript, EXT:%s/Configuration/TypoScript\');';
    ...
    {
        $query = 'INSERT INTO `sys_file_storage` (`pid`, `tstamp`, `crdate`, `name`, `description`, `driver`, ' .
            '`configuration`, `is_default`, `is_browsable`, `is_public`, `is_writable`, `is_online`, ' .
            '`processingfolder`) VALUES 	(0, %d, %d, \'%s assets\', \'Access to site asset files\', \'Local\', ' .
     ...
     */
    protected function kickstartProviderExtension(
        $extensionKey,
        $vendorExtensionKey,
        $author,
        $title,
        ...
        if (false === array_key_exists($extensionKey, $extensions)) {
            ...
            $generator = $extensionService->buildProviderExtensionGenerator(
                $vendorExtensionKey,
                $author,
                $title,
                $description,
